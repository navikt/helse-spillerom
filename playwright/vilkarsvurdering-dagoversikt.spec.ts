...
test.describe('Vilkårsvurdering og Dagoversikt', () => {
    ...
    test('Kan vurdere vilkår og endre dager til avvist', async ({ page }, testInfo) => {
        ...
        await test.step('Naviger til Vilkårsvurdering', async () => {
            ...
            await expect(page.getByRole('row', { name: 'Kapittel 2 - Medlemskap i folketrygden Ikke vurdert Vis mer' })).toBeVisible()
        })
    ...
    await validerAxe(page, testInfo)
        ...
        await test.step('Utvid Kapittel 2 - Medlemskap i folketrygden', async () => {
            ...
            const kapittel2Row = page.getByRole('row', { name: 'Kapittel 2 - Medlemskap i folketrygden Ikke vurdert Vis mer' })
        ...
        await test.step('Åpne endre dager-skjema', async () => {
            ...
            await expect(page.getByRole('heading', { name: 'Velg én eller flere dager du vil endre i tabellen ovenfor' })).toBeVisible()
        })
        ...
        await test.step('Velg dag som skal endres', async () => {
            ...
            await expect(page.getByRole('heading', { name: 'Fyll inn hva de 1 valgte dagene skal endres til' })).toBeVisible()
        })
        ...
        await test.step('Verifiser at endringen er lagret', async () => {
            ...
            const avvistRow = page.getByRole('row', { name: '01.09.2025 Avvist - SB - - - - Den sykmeldte er ikke medlem i folketrygden' })
