    ...
    return (
        <>
            {vilkårMedVurderteÅrsaker.map((vilkår) => {
                ...
                const vurderteOppfylt = vilkår.oppfylt.filter((årsak) => alleVurderteÅrsakskoder.has(årsak.kode))
                ...
                const relaterteVurderinger = vurderinger.filter((vurdering) =>
                    vurdering.underspørsmål.some((usp) => alleVurderteÅrsaker.some((vÅ) => vÅ.kode === usp.svar)),
                )
                    ...
                    <Table.Row key={vilkår.vilkårskode}>
                        <Table.DataCell>
                            <div>
                                ...
                                <div className="mb-2 text-sm text-ax-neutral-700">{vilkår.beskrivelse}</div>
                                <Tag
                                ...
                                {vilkår.vilkårshjemmel && (
                                    <div className="text-xs text-ax-neutral-600">
                                        ...
                                        {vilkår.vilkårshjemmel.setning && ` setning ${vilkår.vilkårshjemmel.setning}`}
                                        {vilkår.vilkårshjemmel.bokstav && ` bokstav ${vilkår.vilkårshjemmel.bokstav}`}
                                    </div>
                                )}
                            </div>
                        </Table.DataCell>
                            ...
                            <div className="space-y-1">
                                {relaterteVurderinger.map((vurdering, index) => (
                                    <div key={index} className="text-sm">
                                        {vurdering.notat && (
                                            <div className="text-ax-neutral-700">{vurdering.notat}</div>
                                        )}
                                    </div>
                                ))}
                            </div>
