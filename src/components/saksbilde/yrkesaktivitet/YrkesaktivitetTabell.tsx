...
export function YrkesaktivitetTabell({ value }: { value: string }): ReactElement {
    ...
    const [yrkesaktivitetTilSlett, setYrkesaktivitetTilSlett] = useState<string | null>(null)
    ...
    const yrkesaktivitetSomIkkeKanKombineres =
        yrkesaktivitet?.filter((aktivitet) => {
    ...
    // Vis advarsel hvis det finnes yrkesaktivitet som ikke kan kombineres og det er flere enn én yrkesaktivitet totalt
    ...
    const handleSlett = (yrkesaktivitetId: string) => {
        setYrkesaktivitetTilSlett(yrkesaktivitetId)
    ...
    const confirmSlett = () => {
        if (yrkesaktivitetTilSlett) {
            ...
            setYrkesaktivitetTilSlett(null)
        }
    }
    ...
    const handleLagreRedigering = (yrkesaktivitetId: string, kategorisering: Record<string, string | string[]>) => {
                            ...
                            <TableBody>
                                {yrkesaktivitet.map((aktivitet, index) => (
                                        ...
                                        content={
                                            redigererId === aktivitet.id ? (
                                                <YrkesaktivitetForm
                                                    ...
                                                    disabled={false}
                                                    initialValues={aktivitet.kategorisering}
                                                    onSubmit={(kategorisering) =>
                                                        handleLagreRedigering(aktivitet.id, kategorisering)
                                                    }
                                                    ...
                                                    {kanSaksbehandles && (
                                                        <HStack gap="2">
                                                            <Button
                                                                ...
                                                                onClick={() => handleRediger(aktivitet.id)}
                                                                disabled={
                                                                    redigererId !== null && redigererId !== aktivitet.id
                                                                }
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => handleSlett(aktivitet.id)}
                                    ...
                                    >
                                        <TableDataCell>
                                            <BodyShort>
                                                {getYrkesaktivitetDisplayText(aktivitet.kategorisering)}
                                            </BodyShort>
                                        </TableDataCell>
                                        <TableDataCell>
                                            <VStack gap="1">
                                                {aktivitet.kategorisering['ORGNUMMER'] && (
                                                    <BodyShort className="text-sm">
                                                        <Organisasjonsnavn
                                                            orgnummer={aktivitet.kategorisering['ORGNUMMER'] as string}
                                                        />
                                                    </BodyShort>
                                                )}
                                            </VStack>
                                        ...
                                        <TableDataCell>
                                            <BodyShort>
                                                {aktivitet.kategorisering['ER_SYKMELDT'] === 'ER_SYKMELDT_JA'
                                                    ? 'Ja'
                                                    : 'Nei'}
                                            </BodyShort>
                                        </TableDataCell>
...
function getYrkesaktivitetDisplayText(kategorisering: Record<string, string | string[]>): string {
