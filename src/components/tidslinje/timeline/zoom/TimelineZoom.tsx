...
function TimelineZoomButton({ label, selected, onSelect }: TimelineZoomButtonProps): ReactElement {
    return (
        <button
            onClick={onSelect}
            className={cn('border-l border-ax-border-neutral p-[6px_9px_6px_8px] text-ax-medium first:border-l-0', {
                'cursor-pointer hover:bg-ax-bg-accent-moderate-hover': !selected,
                'bg-ax-bg-neutral-strong-pressed': selected,
            })}
        >
            <span className={cn({ 'text-ax-text-neutral-contrast': selected })}>{label}</span>
        </button>
    )
}
